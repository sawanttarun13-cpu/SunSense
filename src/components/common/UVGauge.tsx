  ...
  return (
    <svg viewBox="0 0 260 200" style={{ width: '100%', maxWidth: 280, overflow: 'visible' }}>
      <defs>
        ...
        <filter id="arcGlowPulse" x="-30%" y="-30%" width="160%" height="160%">
          <feGaussianBlur stdDeviation="8" result="blur" />
          <feMerge><feMergeNode in="blur" /><feMergeNode in="SourceGraphic" /></feMerge>
        </filter>
      </defs>

      <path d={arcPath} fill="none" stroke={theme === 'dark' ? '#334155' : '#F1F5F9'} strokeWidth={sw} strokeLinecap="round" />
      ...
      {ticks.map(({ uv, inner, outer }) => (
        <line
          ...
          stroke={theme === 'dark' ? '#64748B' : '#CBD5E1'} strokeWidth={1} opacity={0.5}
        />
      ))}

      <text x={cx} y={cy - 10} textAnchor="middle" fontSize={48} fontWeight={700}
        fill={theme === 'dark' ? '#F8FAFC' : '#1E293B'} fontFamily="Poppins, sans-serif" style={{ transition: 'fill 0.4s' }}>
        {value.toFixed(1)}
      </text>
      <text x={cx} y={cy + 12} textAnchor="middle" fontSize={10} fill={theme === 'dark' ? '#94A3B8' : '#94A3B8'} fontWeight={600}
        fontFamily="Poppins, sans-serif" style={{ letterSpacing: '0.05em' }}>
        UV INDEX
      </text>

      <rect x={cx - 35} y={cy + 22} width={70} height={18} rx={9}
        fill={theme === 'dark' ? 'rgba(255,255,255,0.05)' : zone.bg} stroke={zone.border} strokeWidth={1} />
      <text x={cx} y={cy + 34} textAnchor="middle" fontSize={10} fontWeight={600}
        fill={theme === 'dark' ? zone.color : zone.text} fontFamily="Poppins, sans-serif">
        {zone.label}
      </text>
    </svg>
  );
