...
export function MiniMetric({ label, value, bar, barColor, description, onClick }: { label: string; value: string; bar: number; barColor: string; description?: string; onClick?: () => void }) {
  return (
    <div
      className="bg-white rounded-2xl p-4 shadow-sm transition-all duration-150 flex flex-col justify-between"
    ...
    >
      ...
      {description && (
        <div className="text-slate-400 mt-1" style={{ fontSize: '0.65rem', lineHeight: '1.2' }}>
          {description}
        </div>
      )}
    </div>
